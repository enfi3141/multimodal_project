# -*- coding: utf-8 -*-
"""
CDGS v7 (`cdg_7.py`): **`CDGS7`**

요구사항 반영:
- 카메라(12-lead) 꼼수 모션 기능 제거 (완전 고정).
- DeformationMLP에 Positional Encoding (주파수 인코딩) 적용하여 가우시안들이 공간적 위치에 따라 완전히 독립적이고 다채롭게 움직이도록 강제.
- (이전 버전의 특성) SH 방향 감쇠 및 4DGS-style 공간 공분산 유지.

Docker (레포 루트에서, bash 기준) — **백그라운드 실행**
--------------------------------------------------------
아래 블록을 그대로 복사해서 실행합니다. `-v` **왼쪽** 경로만 본인 서버의 PTB-XL
`ptb-xl/1.0.3` 폴더로 바꾸세요.
...
def dipole_position_in_unit_ball(p0: torch.Tensor, p_vel: torch.Tensor, dt: torch.Tensor) -> torch.Tensor:
    ...
    (Updated to remove unit ball restriction)"""
    # (B,N,3,T)
...
class DeformationMLP(nn.Module):
    ...
    (v7 Update): Includes Positional Encoding for high-frequency spatial variation.
    """
    ...
    def __init__(self, d_latent: int = 16, d_hidden: int = 64):
        ...
        self.L_pos = 4
        self.L_t = 2
        d_in = (3 + 3 * 2 * self.L_pos) + (1 + 1 * 2 * self.L_t) + d_latent
    ...
    def forward(
        ...
        # 1) Pre-compute pos_enc on (B, N, 3) ONLY, avoiding 1000x redundant sin/cos
        pos_enc = [canonical_pos]
        for i in range(self.L_pos):
            pos_enc.append(torch.sin((2**i) * math.pi * canonical_pos))
            pos_enc.append(torch.cos((2**i) * math.pi * canonical_pos))
        pos_features_base = torch.cat(pos_enc, dim=-1) # (B, N, 27)

        # 2) Pre-compute t_enc on (B, N, T, 1)
        dt = (t.view(1, 1, T) - mu.unsqueeze(-1)).unsqueeze(-1)
        t_enc = [dt]
        for i in range(self.L_t):
            t_enc.append(torch.sin((2**i) * math.pi * dt))
            t_enc.append(torch.cos((2**i) * math.pi * dt))
        t_features = torch.cat(t_enc, dim=-1) # (B, N, T, 5)

        # We must chunk across N using checkpoint to discard massive intermediate T-dimensional activations.
        # Slice the weights of the first Linear layer once globally
        W = self.net[0].weight
        b = self.net[0].bias
        W_pos = W[:, :pos_features_base.shape[-1]]
        W_t = W[:, pos_features_base.shape[-1] : pos_features_base.shape[-1] + t_features.shape[-1]]
        W_lat = W[:, pos_features_base.shape[-1] + t_features.shape[-1]:]

        def _forward_chunk(c_pos_base, c_t_feat, c_lat):
            # Compute static features efficiently
            c_h_pos = F.linear(c_pos_base, W_pos)  # (B, chunk, d_hidden)
            c_h_lat = F.linear(c_lat, W_lat)       # (B, chunk, d_hidden)
            c_h_static = c_h_pos + c_h_lat + b     # (B, chunk, d_hidden)
            
            # Compute time features
            c_h_t = F.linear(c_t_feat, W_t)        # (B, chunk, T, d_hidden)

            # Broadcast and add
            c_h = c_h_static.unsqueeze(2) + c_h_t  # (B, chunk, T, d_hidden)
            return self.net[1:](c_h)               # (B, chunk, T, 3)

        chunk_size = 512
        deltas = []
        for i in range(0, N, chunk_size):
            c_pos_base = pos_features_base[:, i:i+chunk_size]
            c_t_features = t_features[:, i:i+chunk_size]
            c_lat = latent[:, i:i+chunk_size]

            if self.training:
                c_delta = torch.utils.checkpoint.checkpoint(
                    _forward_chunk, 
                    c_pos_base, c_t_features, c_lat, 
                    use_reentrant=False
                )
            else:
                c_delta = _forward_chunk(c_pos_base, c_t_features, c_lat)
            deltas.append(c_delta)

        delta = torch.cat(deltas, dim=1) # (B, N, T, 3)
...
class GaussianRenderer(nn.Module):
    ...
    def forward(self, mu, sigma0, sigma_vel, amplitude, sh_base, sh_vel, p_pos, scales, rot6d, T: int):
        ...
        dt = t - mu.unsqueeze(-1)
        sigma_t = F.softplus(sigma0.unsqueeze(-1) + sigma_vel.unsqueeze(-1) * dt) + 0.02
...
class CDGS7Loss(nn.Module):
    """CDGS v7 Loss with diversity regularisation."""
