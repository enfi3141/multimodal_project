"""
CDGS v2 (`cdg_2.py`): **`CDGS2`** 클래스.
`cdgs.py`와 동일 구현에서 분리한 포크 — 서버에서는 `--model cdg_2` 로 학습.
이후 v2만 실험할 때는 이 파일을 수정하면 됩니다.
"""
import torch
...
class GaussianPredictor(nn.Module):
    ...
    Per Gaussian:
      ...
      - p0_delta, p_vel: p(t) = normalize(p0_anchor + p0_delta + p_vel * dt)  (learnable anchor)
    """
...
# ═══════════════════════════════════════════════════════════════════════════
# 6. Main CDGS2 Class (cdgs v2)
# ═══════════════════════════════════════════════════════════════════════════
class CDGS2(nn.Module):
    def __init__(
        self,
        d_model=256,
        n_gaussians=128,
...
# ═══════════════════════════════════════════════════════════════════════════
# 7. CDGS2Loss + describe_gaussians
# ═══════════════════════════════════════════════════════════════════════════
class CDGS2Loss(nn.Module):
    """
    CDGS2 전용 손실: fine/coarse L1 정합만.
    train_all_models.py: cdg_2 일 때만 CDGS2Loss() 사용.
    네 번째 인자(amplitude)는 train 스크립트 positional 호환용(미사용).
    """
    ...
    def forward(
        ...
        total = self.w_recon * losses["recon"] + self.w_coarse * losses["coarse"]
        losses["total"] = total
        return total, losses
...
if __name__ == '__main__':
    ...
    model = CDGS2(d_model=256, n_gaussians=128, use_metadata=True)
    
    print(f"CDGS2 Parameters: {sum(p.numel() for p in model.parameters()):,}")
