...
def run_experiment(args, label_idx, selected_leads, lead_setting, device):
    print("=" * 100)
    print(f"Fixed lead subset training | Label: {label_idx} ({LABELS[label_idx]})")
...
def main():
    parser = argparse.ArgumentParser(description="PTB-XL fixed lead subset AUC training")
    ...
    parser.add_argument("--selected_leads", type=int, nargs="+", required=True)
    parser.add_argument("--lead_setting", type=str, default="greedy-fixed")
    ...
    parser.add_argument("--save_csv", type=str, default="./results/fixed_subset_auc_results.csv")
    ...
    for lead_idx in args.selected_leads:
        if not (0 <= lead_idx < len(LEADS)):
            raise ValueError("--selected_leads must contain indices from 0 to 11")

    run_experiment(
        args=args,
        label_idx=args.label_idx,
        selected_leads=args.selected_leads,
        lead_setting=args.lead_setting,
        device=device,
    )
